...
- OK/pass センチネルを複数カテゴリで使う場合、発火条件がカテゴリ間で統一されており、スコア分母が意図せず増加しないか
- grade / score / rank を出力するツールでは、スコア分子と分母がすべてのカテゴリで一貫しているか。特定カテゴリ（entry-point 等）が分母にのみ計上されて分子から除外されている場合、その理由が summary または凡例で説明されているか
