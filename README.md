...
- 機能と UI を分離: UI に機能を埋め込まず、ローカルでも呼び出し可能な構造を優先する
...
- `builder`: 実装時は機能本体を callable logic として切り出し、UI はその呼び出し層として組む
...
- UI や動画の質もプロダクト品質の一部
- UI は機能本体を呼び出す層として設計し、機能ロジックは別枠で持つ
- ローカル用途では API サーバー化までは不要だが、将来 UI 差し替えや自動実行に使える呼び出し境界は作る
- 再利用するのは原則や判断観点
